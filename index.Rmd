...
#DART Rail stops
DART_Rail <- st_read("E:/CPAL Dropbox/Data Library/DART/LightRailStations.shp") %>%
  st_transform(crs = 4326)

#DART Bus stops
DART_Bus <- st_read("E:/CPAL Dropbox/Data Library/DART/BusStops_2018.shp") %>%
  st_transform(crs = 4326)
...
wra_pal <- colorBin("BuPu", WRA$women_per, 5, pretty = TRUE)
...
* **Car Travel Time:** Raster layer visualizing the average travel time to a Trust Her clinic within a 15 minute drive time.
...
leaflet() %>%
  ...
  addCircleMarkers(data = DART_Bus,
                   ...
                   group = "DART Bus") %>%
  addLayersControl(
    #baseGroups = c("Car Travel Time", "Women (15 to 49)"),
    overlayGroups = c("Trust Her Clinics", "Car Travel Time", "Women (15 to 49)", "DART Bus", "DART Station"),
    position = "topright",
    options = layersControlOptions(collapsed = FALSE)) %>%
  hideGroup(c("DART Bus", "DART Station", "Women (15 to 49)"))
```